...
<form name="frm" action="<c:url value='/sub03/sub02_01.do'/>" method="post">
    ...
    <div class="searchTop type02">
        <p class="list_info"><b><c:out value="${totCnt}"/></b>개의 게시물이 등록되어 있습니다</p>
        <div class="searchBox">
            ...
            <span>
		<label for="searchstring" class="all-hidden"><span>검색어 입력</span></label>
		<input type="text" placeholder="검색어를 입력하세요" id="searchWrd" name="searchWrd" style="ime-mode:active;"
               title="검색어를 입력해주세요" value="<c:out value="${searchVO.searchWrd}"/>"/>
...
<div class="report_li">
    <ul>
        <c:forEach var="result" items="${resultList}" varStatus="status">
            <li>
                <span class="select_btn"><label for="c01" class="all-hidden"></label></span>
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <div class="photo">
                            <span class="tag <c:out value="${result.category}"/>">진행중</span>
                            <c:choose>
                                <c:when test="${result.fileSn ge 0}">
                                    <img src="<c:url value='/cmm/fms/getImage.do?fileSn=${result.fileSn}&atchFileId=${result.atchFileId}'/>"
                                         alt=""/>
                                </c:when>
                                <c:otherwise>
...
<script type="text/javascript">
    ...
    function fn_egov_inqire_BBS(nttId, bbsId) {
        ...
        document.submitParam.action = "<c:url value='/sub03/sub02_01v.do'/>";
        document.submitParam.submit();
    }

    function fn_egov_select_BBSList(pageNo) {
        document.frm.pageIndex.value = pageNo;
        document.frm.action = "<c:url value='/sub03/sub02_01.do'/>";
        document.frm.submit();
    }
