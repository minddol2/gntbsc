...
function fn_egov_select_List(pageNo) {
	...
	document.frm.action = "<c:url value='/sb06/sub01.do'/>";
	document.frm.submit();
}
...
function fn_egov_select_dele() {
	...
	if(item_cnt == 0) {
		...
		document.delfrm.action = "<c:url value='/sb06/sub01_sdele.do'/>";
...
function fn_egov_change_status() {
	...
	if(item_cnt == 0) {
		...
		document.delfrm.action = "<c:url value='/sb06/sub01_schange.do'/>";
		document.delfrm.submit();
	}
}
//-->
...
<h4 class="tit_dot">일자리 상담신청 목록</h4>

<form id="detailvfrm" target="detailvfrm" method="post" action="<c:url value='/sb06/sub01_write.do'/>" >
...
<table class="dash_table tal mb">
	...
	<tr>
		<th>구분</th>
		<td>
			<input type="radio" name="counsel_gbn" id="counsel_gbn" value="" <c:if test="${'' eq searchVO.counsel_gbn}">checked</c:if> /><label for="counsel_gbn">전체</label>
			<input type="radio" name="counsel_gbn" id="counsel_gbn1" value="1" <c:if test="${'1' eq searchVO.counsel_gbn}">checked</c:if> /><label for="counsel_gbn1">구인</label>
			<input type="radio" name="counsel_gbn" id="counsel_gbn2" value="2" <c:if test="${'2' eq searchVO.counsel_gbn}">checked</c:if> /><label for="counsel_gbn2">구직</label>
		</td>
	</tr>
	<tr>
		<th>상태</th>
...
<table class="comm_table tac bmb">
	...
	<tr class="HighlightableRow">
		...
		<td><a href="#" onclick="fn_egov_write('${result.idx}');"><c:out value="${result.reg_date}"/></a></td>
		<td>
	...
	<c:if test="${fn:length(job_list) == 0}">
	<tr>
		<td colspan="8">일치하는 자료가 없습니다.</td>
	</tr>	
	</c:if>
	</tbdoy>
</table>
