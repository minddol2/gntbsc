...
function fn_egov_inqire_BBS(nttId, bbsId) {
	 ...
	 document.submitParam.action = "<c:url value='/sb02/sub04_view.do'/>";
	 document.submitParam.submit();
}

function fn_egov_select_BBSList(pageNo) {
	document.frm.pageIndex.value = pageNo;
	document.frm.action = "<c:url value='/sb02/sub04.do'/>";
	document.frm.submit();
}
//-->
</script>

<h4 class="tit_dot">FAQ</h4>



	<form name="frm" action="<c:url value='/sb02/sub04.do'/>"
		method="post">
	...
	<div class="basicList">
		<ul>
			...
			<li>
						...
						<span class="title">
							<form name="subForm" method="post" action="<c:url value='/sb02/sub04_view.do'/>">
	...
	<div class="boardButton">
		<span><a href="<c:url value='/sb02/sub04.do'/>">목록</a> </span> 
		<c:if test="${us_level le 2}">
		<span><a href="<c:url value='/sb02/sub04_write.do'/>?bbsId=<c:out value="${boardVO.bbsId}"/>">글쓰기</a> </span> 
		</c:if>
	</div>
...
<form name="frm" action ="<c:url value='/sb02/sub04.do'/>" method="post">
