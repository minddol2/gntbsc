...
<script type="text/javascript">
	...
	function fn_egov_delete_BBS() {
        if (confirm('<spring:message code="common.delete.msg" />')) {
            document.frm.action = "<c:url value='/sb02/sub01_delete.do'/>";
            document.frm.submit();
        }   
    }
    
    function fn_egov_moveUpdt_BBS() {
        document.frm.action = "<c:url value='/sb02/sub01_modify.do'/>";
        document.frm.submit();          
    }
...
<div id="viewWrap">
	<div class="viewTop">
		...
		<ul>
			<li><span class="name">사이즈(가로) <c:out value="${result.sizex}" /></span></li>
			<li><span class="date">사이즈(세로) <c:out value="${result.sizey}" /></span></li>	
		</ul>
		<ul>
			<li><span class="name">위치(X) <c:out value="${result.leftx}" /></span></li>
			<li><span class="date">위치(Y) <c:out value="${result.lefty}" /></span></li>	
		</ul>
	</div>
