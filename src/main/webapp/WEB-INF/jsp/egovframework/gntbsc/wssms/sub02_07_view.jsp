...
<script type="text/javascript">
	...
	function fn_egov_select_BBSList(pageNo) {
		...
		document.frm.action = "<c:url value='/sb02/sub07.do'/>";
		document.frm.submit();
	}
	
	function fn_egov_delete_BBS() {
        if (confirm('<spring:message code="common.delete.msg" />')) {
            document.frm.action = "<c:url value='/sb02/sub07_delete.do'/>";
            document.frm.submit();
        }   
    }
    
    function fn_egov_moveUpdt_BBS() {
        document.frm.action = "<c:url value='/sb02/sub07_modify.do'/>";
        document.frm.submit();          
    }
    
    function fn_egov_addReply_BBS() {
        document.frm.action = "<c:url value='/sb02/sub07_reply.do'/>";
        document.frm.submit();          
    } 
...
<div id="viewWrap">
	<div class="viewTop">
		...
		<ul>
			...
			<li><span class="date"><c:out value="${result.ntceBgnde}"/>~<c:out value="${result.ntceEndde}"/></span></li>	
			<li><span class="view">조회수 <c:out value="${result.inqireCo}" /></span></li>	
		</ul>
	</div>
	...
	<div class="v_attem">
		<ul>
			<li>새창 : <c:out value="${result.nttCn}" escapeXml="false" /></li>
			<li><a href="<c:out value="${result.link_url}" />" target="_blank"><c:out value="${result.link_url}" /></a></li>
		</ul>
	</div>
	...
	<div class="boardButton">
		...
		<span><a href="<c:url value='/sb02/sub07.do'/>">목록</a> </span> 
	</div>
</div>
<!--뷰 e-->
